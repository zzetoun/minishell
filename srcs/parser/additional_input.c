...
static int	write_heredoc_block_1(int fd, const char *delim, bool quoted)
{
    char	*l;
    char	*out;
    ...
    for (;;)
    {
        ...
        out = (quoted ? ft_strdup(l) : setup_env_in_line(l));
        if (!quoted)
            free(l);
...
static char	*handle_heredoc_3(char *line, int idx)
{
    char	*pos;
    char	*delim;
    char	quote_char;
    ...
    write_heredoc_block_1(fd, delim, quoted);
    close(fd);
    free(delim);
